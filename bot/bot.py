                ...
                if text == "/picks":
                    ...
                    if str(chat_id) == str(ADMIN_ID):
                        reply(chat_id, (
                            ...
                            "⚙️ *Settlement Commands (Admin)*\n"
                            "/forcesettle — Run full settlement pass now\n"
                            "/debugsettle — BDL lookup test for all pending picks\n"
                            "/manualgrade 268 win 6 — Grade pick #268 as win (actual=6)\n"
                            "/manualgrade 268 loss — Grade pick #268 as loss\n"
                            "/manualgrade 268 void — Void pick #268\n\n"
...
def _build_cross_game_parlay(pool):
    ...
    safe_size = len(script_pool)
    bal_size  = len(script_pool)
    agg_size  = len(script_pool)
...
def _register_commands():
    ...
    admin_commands = public_commands + [
        ...
        {"command": "debugsettle",    "description": "BDL lookup test for all pending picks"},
        {"command": "manualgrade",    "description": "Grade a pick by ID: /manualgrade 268 win 6"},
